[English](README.md) | **한국어**

# BC-250 Custom Pannel

AMD BC-250에서 Bazzite를 사용할 때 GPU 상태, 펌웨어 정보와 성능 설정을 한 화면에서 확인하고 조절하는 GTK4 제어판입니다. 프로젝트 이름의 `Pannel` 철자는 기존 요청 경로와 호환하기 위해 유지합니다.

이 프로젝트는 Python 화면만 제공하지 않습니다. 실행에 필요한 GPU 거버너, CU 부팅 관리자, Bazzite 43용 UMR 실행본, 서비스와 정책 템플릿을 버전과 SHA-256으로 고정해 함께 제공합니다.

![BC-250 Control Panel 대시보드](screenshot.png)

위 이미지는 Bazzite GNOME에서 실행한 실제 화면입니다. 표시되는 온도·전력·클럭·팬·CPU/CU 값은 장치와 현재 부하에 따라 달라집니다.

## 주요 기능

- 상단 요약에서 현재 CPU 코어/스레드와 GPU CU를 동시에 표시
- CPU·GPU 온도, GPU 전력·클럭·전압과 팬 RPM을 1초 간격으로 갱신
- 절전·균형·성능 프리셋과 거버너의 전체 `u32` 입력 범위를 쓰는 사용자 GPU 클럭·mV 상한 설정
- 스로틀 온도와 복구 온도 설정 및 동봉 전압 곡선 사용
- 기본 24 CU는 고정하고 선택 CU 16개를 WGP별로 고르며 선택·실시간·저장·레지스터 검증 상태 표시
- 6C/12T와 8C/16T 전환 및 완전 전원 차단 뒤 1회만 수행하는 안전한 웜 재부팅 복구
- 자동 절전과 화면 꺼짐의 차단·시간 프리셋·1–240분 사용자 설정
- Control·CPU/GPU Cores·Power/IDLE 초안을 전역 `적용` 또는 `저장` 버튼 한 번으로 처리
- BIOS·커널 정보, 한국어·영어·일본어·중국어 UI
- 실행본·설정·서비스·정책·라이선스 파일을 검사하고 빠진 항목을 한 번에 설치

## 지원 대상

- AMD BC-250 (`1002:13fe`)
- Bazzite x86_64
- Bazzite GNOME 또는 Deck GNOME 이미지
- Wayland 세션

다른 GPU, 일반 Fedora, Ubuntu, Arch/CachyOS는 설치 대상이 아닙니다. 프로그램은 지원 환경을 먼저 검사하고 일치하지 않으면 시스템 설치를 중단합니다.

## 빠른 시작

Git에서 받은 프로젝트 폴더에서 다음 실행 파일을 엽니다.

```bash
./run.sh
```

GUI는 먼저 열립니다. 상단 구성요소 줄은 항상 표시되며 긴 구성요소 목록은 잘리지 않고 자동으로 줄바꿈됩니다. 거버너·CU 관리자·UMR·권한 도우미·CPU 모드 서비스뿐 아니라 systemd 단위, D-Bus 정책, Polkit 규칙과 라이선스 파일이 빠졌거나 설치본 해시가 동봉 버전과 다를 때 `구성요소 설치` 버튼이 활성화됩니다. 버튼을 한 번 누르고 시스템 인증을 완료하면 모든 번들 파일의 SHA-256을 확인한 뒤 매니페스트에 선언된 시스템 파일 14개를 고정된 위치와 권한으로 설치·갱신합니다. Bazzite가 일반 사용자의 `/etc/polkit-1/rules.d` 접근을 막아도 GUI가 Polkit 규칙을 검증할 수 있도록 root 소유의 읽기 전용 설치 영수증도 기록합니다. 호환 구성요소가 모두 설치되어 있으면 설치 버튼은 비활성화됩니다.

GNOME 앱 목록에 등록하려면 다음 파일을 한 번 실행합니다.

```bash
./install-app.sh
```

이 스크립트는 현재 Git 클론 위치를 자동으로 사용하므로 사용자 이름이나 설치 경로를 고정하지 않습니다.

## 화면 구성

### 컴팩트 실시간 대시보드

창 너비는 520픽셀이고 선택 언어에 따라 높이가 자동으로 맞춰지는 컴팩트한 구성입니다. 세로 스크롤 컨테이너를 사용하지 않으며 실시간 상태, 거버너, CPU/GPU 코어 제어, 전원 설정, BIOS와 커널 정보가 한 화면에 모두 표시됩니다. CPU/GPU 영역은 필요한 폭만 사용하고 WGP 표가 남은 가로폭을 채우므로 오른쪽에 불필요한 빈 공간을 남기지 않습니다. 여섯 센서는 하나의 2행 상태 밴드에 다음 순서로 표시됩니다.

제목표시줄에는 프로그램 이름만 표시합니다. 최상단 개요는 왼쪽에 현재 CPU/GPU 상태, 오른쪽에 언어 선택기를 두고, 그 아래에 가용 상태·BIOS 버전·커널 버전을 동일한 너비의 세 칸으로 배치했습니다. 별도 하단 줄을 없애 전역 적용·저장 버튼이 화면의 맨 아래가 되도록 구성했습니다. 가용 상태는 테두리와 배경 없는 색상 텍스트로 한 번만 표시합니다. 주요 영역은 기능별로 서로 다른 강조색을 사용하고 버튼·콤보박스 텍스트는 어두운 배경에서 읽을 수 있도록 흰색으로 통일했습니다.

1. CPU 온도
2. GPU 온도
3. GPU 전력
4. GPU 클럭
5. SMU 전압
6. 동작 중인 팬 RPM

GPU 코어 영역은 옆의 CPU 표기와 같은 숫자 우선 형식인 `24 / 40 CU 활성화`로 전체 40개 중 실제 활성 수를 표시합니다. 체크 선택이 실제값과 다를 때에만 같은 줄에 변경 대기 값을 추가합니다. CPU 코어 영역은 현재 `6C / 12T` 또는 `8C / 16T` 상태와 양방향 동작 토글 하나만 표시합니다. 완전 전원 차단 후 실제 상태와 저장된 8C/16T 설정이 다르면 반복 문장을 추가하지 않고 토글 도움말로 알립니다.

센서는 1초마다 백그라운드에서 갱신되며 느린 시스템 호출 때문에 창이 멈추지 않도록 구성했습니다. 거버너·스로틀·복구 온도와 CU 입력칸은 시작할 때 한 번만 현재 설정으로 채우며 이후 센서 갱신이 사용자가 입력한 값을 덮어쓰지 않습니다. BC-250의 GPU 클럭과 드라이버가 보고하는 소프트웨어 전력 센서는 부하 중 실제 소비전력계와 차이가 날 수 있습니다. 드라이버가 센서를 제공하지 않으면 임의의 값을 만들지 않고 `확인 불가`로 표시합니다.

### 언어

언어를 `자동`으로 두면 `LC_ALL`, `LC_MESSAGES`, `LANG` 순서로 시스템 언어를 판단합니다. 최상단 개요의 오른쪽에서 한국어, English, 日本語, 简体中文을 직접 선택할 수 있고, 지원하지 않는 로캘 및 번역 키가 없는 경우에는 English를 사용합니다. 선택값은 `~/.config/bc250-custom-pannel/settings.json`에 저장되며 재시작 없이 즉시 화면에 반영됩니다.

### CPU 및 팬 센서 안전

CPU 온도는 먼저 `k10temp`의 `Tctl`을 읽고, 없으면 보드 센서의 `CPU`를 사용합니다. 팬 RPM은 `nct6686` 또는 `nct6687` 센서에서 읽습니다. `hwmon` 번호를 고정하지 않고 이름과 레이블로 매번 찾습니다. 이 릴리스에서 애플리케이션은 PWM 값을 절대 쓰지 않으며 팬 제어는 엄격히 읽기 전용입니다. `Pump Fan`에 연결된 물리 장치가 무엇인지 확인되기 전까지 팬 제어는 범위 밖입니다.

### 전원 / IDLE

`시스템 절전`과 `화면 꺼짐` 콤보박스는 GNOME의 세션 타이머만 제어합니다. `사용 안 함`, 5·10·15·30·60분 프리셋과 `사용자 설정`을 제공하며, 선택값은 전역 `적용` 또는 `저장` 버튼을 누르기 전까지 초안으로만 유지됩니다. 사용자 설정을 선택하면 1–240분 입력칸이 나타납니다. 두 항목을 사용하지 않아도 CPU의 MWAIT 유휴 대기와 AMDGPU DPM 자동 클럭·전력 관리는 꺼지지 않습니다.

현재 BC-250에서는 cpuidle 드라이버 파일이 `none`으로 보이더라도 커널 로그와 CPU 기능에서 MWAIT 사용이 확인될 수 있습니다. 이 경우 GUI는 `CPU MWAIT`로 표시합니다. GPU가 `auto`로 표시되면 부하에 따라 클럭과 전력을 낮추는 자동 관리가 동작 중입니다.

자동 절전 시간은 AC·배터리 세션에 함께 저장되고, 화면 꺼짐 시간은 GNOME 유휴 타이머로 저장됩니다. `차단`은 해당 타이머를 0으로 설정합니다. 이 설정은 사용자 세션 설정이며 관리자 인증이나 재부팅이 필요하지 않습니다.

### BIOS 및 커널

- BIOS 제조사
- BIOS 버전
- BIOS 날짜
- Linux 커널 버전
- 시스템 아키텍처

정보는 `/sys/class/dmi/id`와 실행 중인 커널에서 읽습니다. 관리자 권한은 필요하지 않습니다.

### 성능과 온도

| 프리셋 | 클럭 범위 | 안전 곡선 상한 | 용도 |
|---|---:|---:|---|
| 절전 | 500–1500 MHz | 900 mV | 낮은 발열과 소비전력 |
| 균형 | 500–1700 MHz | 920 mV | 성능과 효율의 균형 |
| 성능 | 500–1800 MHz | 930 mV | 현재 장비에서 검증한 최대 설정 |

모든 카드 아래의 전역 버튼 두 개가 이전 카드별 적용·저장 버튼을 대신합니다. `적용`은 전체 초안을 먼저 검증한 뒤 현재 세션에 거버너, 정확한 WGP 마스크, CPU 모드와 GNOME 전원 타이머를 적용합니다. `저장`은 같은 내용을 적용하면서 거버너·WGP·CPU 선택을 부팅 설정에도 기록합니다. 설치 구성요소에 업데이트가 필요해도 두 버튼은 활성 상태를 유지하고, 별도의 구성요소 줄에서 업데이트 필요 상태와 설치 작업을 처리합니다. 거버너의 최소 클럭·최대 클럭·mV 상한·스로틀 온도·복구 온도는 하나의 D-Bus 호출로 전달되며, 영구 파일은 백업 후 원자적으로 씁니다.

프리셋 아래의 `사용자 설정`을 고르면 거버너 D-Bus 인터페이스의 부호 없는 32비트 필드와 동일하게 최소·최대 GPU 클럭을 0~4,294,967,295 MHz 범위에서 직접 지정할 수 있습니다. 클럭의 `0`은 해당 경계를 제한하지 않는다는 뜻입니다. 두 값이 모두 0이 아니면 최소값이 최대값을 넘을 수 없지만, 프로그램은 기존의 500–1800 MHz 범위나 100 MHz 간격을 강제하지 않습니다. mV 입력은 전압 상한입니다. 거버너는 각 클럭에서 동봉 곡선값과 사용자가 고른 상한 중 더 낮은 값을 사용합니다. `0 mV`는 추가 상한을 끄고 기존 곡선을 그대로 사용하며, 높은 상한을 입력해도 곡선 포인트를 강제로 올리지 않습니다. 동봉 곡선은 350–2400 MHz를 제공하고 기본 작동 범위는 500–1800 MHz입니다. 2400 MHz를 넘는 값은 해당 전압 포인트를 설정 파일에 추가한 경우에만 적용할 수 있습니다.

스로틀 온도와 복구 온도는 각각 0~255°C를 입력할 수 있습니다. 수정 동봉 거버너도 같은 범위를 독립적으로 처리하므로 대소관계나 간격을 강제하지 않으며, `0`은 해당 임계값을 끕니다. 기본값은 85°C/75°C입니다. 저장 후 서비스 재시작이 실패하면 이전 설정으로 복원합니다.

## 안전 범위

- 사용자 클럭 입력은 부호 없는 32비트 전체 범위를 허용합니다. `0`은 열린 경계이며 두 경계가 모두 0이 아니면 최소값을 먼저 입력해야 합니다.
- 스로틀·복구 온도는 프로그램이 별도 간격을 강제하지 않고 각각 0~255°C를 허용합니다.
- mV 입력은 부호 없는 32비트 전체 범위를 허용하지만 고정 전압 명령이 아니라 동봉 곡선에 씌우는 상한입니다. `0`은 추가 상한 없음입니다.
- 프로그램은 사용자가 고른 클럭이나 전압이 모든 BC-250 보드에서 안정적이라고 보증하지 않습니다.
- 정확한 와트 제한 장치가 아니므로 전력은 센서로 표시하고 클럭·전압 프리셋으로 간접 제한합니다.
- 성능 설정은 기존 `cyan-skillfish-governor-smu` 한 개만 사용합니다. 다른 GPU 거버너와 동시에 실행하지 마십시오.
- 변경한 WGP를 적용하면 실제 GPU 레지스터를 쓰고 정확한 값을 다시 읽어 검증합니다. 불안정한 CU에서는 화면이나 작업이 멈출 수 있으므로 먼저 작업을 저장하십시오.

### WGP 개별 선택과 실제값 검증

GPU 네 행에는 각각 WGP 셀 다섯 개가 있습니다. WGP 0–2는 기본 24 CU이므로 끌 수 없고, 각 행의 WGP 3·4는 개별 선택할 수 있습니다. 따라서 24·32·40 고정 프로필뿐 아니라 24~40 사이의 모든 짝수 CU 구성을 만들 수 있습니다. GUI는 현재 초안, 실제 레지스터값, 저장된 부팅 마스크를 구분하며, 총 CU 수가 우연히 같아도 정확한 마스크가 다르면 실패로 표시합니다.

`적용`은 선택 마스크를 쓴 뒤 네 레지스터가 모두 정확히 일치할 때만 성공합니다. `저장`은 `/etc/bc250-cu-live-manager.conf`도 백업·갱신해 같은 마스크를 부팅 때 요청합니다. 공장 기본 24 CU 칸은 항상 켜지는 고정 항목이므로 비활성 상태로 명확히 표시하고 선택 가능한 추가 CU만 조작할 수 있습니다. 관리자는 `/run/bc250-custom-pannel/cu-state.json`에 상태를 원자적으로 기록하고, GUI는 서비스 로그만 보고 40 CU라고 추정하지 않습니다. 선택 CU는 보드마다 불안정하거나 불량일 수 있으므로 각 조합을 실제 부하로 확인해야 합니다.

### CPU 8코어 / 16스레드 토글

고정 문구 `8C / 16T 사용` 스위치는 전역 작업에 포함되는 초안입니다. 꺼짐은 6C/12T, 켜짐은 8C/16T 요청을 뜻합니다. 기본 `6C / 12T`에서 켜면 동봉된 `bc250-cu-live-manager --yes cpu-unlock`을 요청하며, 추가 코어는 재부팅 뒤 나타납니다. 일시적인 실제 코어 수가 저장된 CPU 선택과 달라도 스위치는 저장된 선택을 유지하며, 사용자가 토글을 직접 바꾼 뒤에는 주기적 새로고침이 그 선택을 덮어쓰지 않습니다. 따라서 다른 카드의 적용·저장을 눌러도 저장된 8C/16T 선택이 6C/12T로 바뀌지 않습니다. 성공한 작업이 재부팅 필요 상태를 반환하면 CPU 여부와 관계없이 일반 완료 안내 대신 명확한 재부팅 경고 메시지박스를 표시합니다.

권한 도우미가 아직 없을 때 CPU 토글을 먼저 눌러도 구성요소 설치와 CPU 모드 적용을 하나의 관리자 작업으로 처리하므로 인증 창을 연달아 두 번 띄우지 않습니다.

`8C / 16T` 상태에서 끄면 검증되지 않은 역방향 SMU 쓰기를 하지 않고 Linux CPU hotplug 인터페이스로 추가 두 물리 코어의 스레드를 오프라인 처리합니다. 요청 상태는 `/etc/bc250-custom-pannel-cpu.conf`에 저장되고 `bc250-cpu-mode.service`가 부팅 때 다시 적용합니다. 중간 쓰기에 실패하면 이미 바꾼 스레드를 원래 상태로 되돌립니다.

최초 구성요소 설치만으로 CPU 상태가 바뀌지 않으며 CPU 모드 서비스를 즉시 시작하지도 않습니다. 초기 모드는 `auto`라서 사용자가 직접 저장하기 전까지 현재 상태를 유지합니다. SMU 언락은 휘발성이므로 완전한 전원 차단 뒤 6C/12T로 돌아갈 수 있습니다. 8C/16T가 저장되어 있으면 부팅 서비스가 콜드 상태를 확인해 언락을 예약하고 웜 재부팅을 정확히 한 번만 수행합니다. 다음 부팅도 6C/12T라면 실패 상태를 기록하고 자동 재부팅을 멈추므로 반복 재부팅하지 않습니다. 선택 코어는 개별 보드에서 불안정하거나 불량일 수 있으므로 부하 테스트는 필요합니다.

## 동봉 구성요소

정확한 설치 파일, 원본 URL, 설치 경로, 모드와 SHA-256은 [`VENDOR-MANIFEST.json`](VENDOR-MANIFEST.json)에 기록됩니다.

| 구성요소 | 고정 버전 | 용도 |
|---|---|---|
| cyan-skillfish-governor-smu | v0.4.11-bc250.2 | 클럭·mV 상한·0–255°C 온도를 원자적으로 적용 |
| bc250-cu-live-manager | 2026-08-11 readback 빌드 | 정확한 WGP 마스크 적용과 레지스터 재검증 |
| UMR | 1.0.10-6.fc43 | BC-250 GPU 레지스터 접근 |
| 권한 도우미 | 0.3.0 | 하나의 전체 설정 초안을 검증·적용 |
| CPU 모드 서비스 | 2 | 저장 CPU 모드와 1회성 콜드 부팅 복구 적용 |

거버너와 UMR은 MIT 라이선스입니다. 거버너 수정 사항은 `vendor/patches/`에 재현 가능한 소스 패치로 포함하고, 원문 라이선스와 저작권 고지는 `vendor/licenses/` 및 [`THIRD_PARTY_NOTICES.md`](THIRD_PARTY_NOTICES.md)에 보존합니다. Polkit 규칙은 읽기 전용 `/usr/share` 대신 `/etc/polkit-1/rules.d/`에 설치합니다. 설치기는 매니페스트 해시가 하나라도 다르면 어떤 시스템 파일도 쓰지 않습니다. 다만 현재 매니페스트에는 디지털 서명이 없으므로 이 검사는 파일 손상과 개별 변경을 감지할 뿐, 저장소와 매니페스트가 함께 변조된 경우까지 출처를 증명하지는 않습니다.

번들 UMR은 Bazzite 43 x86_64에서 검증했습니다. CU 부팅 서비스는 설치된 `/opt/bc250-custom-pannel/bin/umr`을 명시적으로 사용합니다. `ldd` 검사에서 공유 라이브러리가 누락되면 해당 실행본을 강제로 사용하지 않습니다. 향후 Bazzite 기반 버전에서는 배포판 UMR 패키지 설치가 필요할 수 있고 rpm-ostree 적용 후 한 번 재부팅해야 할 수 있습니다.

## 설치 범위와 관리자 권한

센서 조회, BIOS·커널 정보 표시, 언어 변경과 GNOME 자동 절전·화면 꺼짐 설정은 일반 사용자 권한으로 동작합니다. `./run.sh`를 실행하는 것만으로 관리자 권한을 얻거나 시스템 파일을 변경하지 않습니다. 다음 작업에만 시스템 인증을 요청합니다.

- 동봉 구성요소 설치와 제거
- 거버너 설정의 영구 저장 및 서비스 재시작
- 다음 부팅 CU 프로필 저장
- CPU 언락 요청과 CPU 온라인·오프라인 모드 저장

시스템 인증을 완료하면 설치기와 권한 도우미는 다음과 같이 고정된 위치만 사용합니다.

| 위치 | 내용 |
|---|---|
| `/etc/cyan-skillfish-governor-smu/` | 거버너 설정과 안전 전압 곡선 |
| `/etc/bc250-cu-live-manager.conf` | 다음 부팅 CU 프로필 |
| `/etc/bc250-custom-pannel-cpu.conf` | 부팅 시 적용할 CPU 모드 |
| `/usr/local/bin/bc250-cu-live-manager` | CU·CPU 관리 도구 |
| `/opt/bc250-custom-pannel/` | 고정 UMR 실행본과 제3자 라이선스 |
| `/usr/local/libexec/bc250-custom-pannel-privileged` | 제한된 관리자 작업 도우미 |
| `/etc/systemd/system/` | 거버너·CU·CPU 서비스 |
| `/etc/dbus-1/system.d/` | 거버너 D-Bus 정책 |
| `/etc/polkit-1/rules.d/` | 관리자 인증 정책 |

서비스는 GPU 레지스터와 시스템 설정에 접근하기 위해 root로 실행되지만 외부 포트를 열거나 네트워크 서버를 시작하지 않습니다. 프로젝트 코드에는 API 키나 계정 비밀번호를 저장하는 기능이 없습니다. GNOME 앱 등록은 현재 사용자의 `~/.local/share/applications/`만 변경합니다.

## 보안 검토 상태

> **공개 배포 판정: 보류.** 아래 내용은 2026-08-02 현재 코드에 대한 점검 결과이며 보안 인증이나 무결점 보증이 아닙니다. 개인 단독 사용에서는 노출 가능성이 비교적 낮지만, 다른 사용자에게 배포하기 전에는 다음 세 항목을 수정해야 합니다.

1. **Polkit 인증 유지 범위 — 높음**
   `vendor/templates/49-bc250-custom-pannel.rules`가 범용 `org.freedesktop.policykit.exec` 동작에 `AUTH_ADMIN_KEEP`을 사용합니다. 이 방식은 관리자 암호를 입력한 뒤 짧은 인증 유지 시간 동안 프로그램 세부 정보가 다른 `pkexec` 실행에도 승인이 재사용될 수 있습니다. 배포 전 `AUTH_ADMIN`으로 바꾸거나 실행 파일이 고정된 전용 Polkit 동작을 정의해야 합니다. [Polkit 공식 문서](https://polkit.pages.freedesktop.org/polkit/polkit.8.html)도 변수에 의존하는 규칙에는 `*_KEEP`을 사용하지 말라고 안내합니다.

2. **거버너 D-Bus 쓰기 권한 — 중간**
   `vendor/templates/com.cyanskillfish.Governor.conf`는 기본 로컬 사용자에게 거버너 쓰기 인터페이스 호출을 허용합니다. 따라서 같은 장치에서 실행되는 다른 프로세스가 GUI의 부호 없는 정수 및 경계 순서 검사를 거치지 않고 상위 거버너 API를 직접 호출할 수 있습니다. 단일 사용자 장치에서는 위험이 낮아지지만 GUI 검증은 시스템 전체의 보안 경계가 아닙니다. 공개 배포 전 전용 사용자 그룹이나 별도 인증 도우미로 쓰기 권한을 제한해야 합니다.

3. **선택적 UMR 소스 설치 — 중간·조건부**
   동봉된 `bc250-cu-live-manager`의 수동 `install-umr` 기능은 특정 커밋이나 릴리스 해시로 고정하지 않은 원격 기본 브랜치를 받아 root 권한으로 빌드·설치할 수 있습니다. GUI의 `구성요소 설치` 버튼은 이 경로를 사용하지 않고 검증된 번들 UMR을 사용합니다. 그래도 배포본에서는 해당 수동 기능을 제거하거나 변경 불가능한 버전과 검증 해시로 고정해야 합니다.

현재 확인된 긍정적인 근거는 다음과 같습니다.

- `VENDOR-MANIFEST.json`에 등록된 동봉 구성요소 17개가 모두 기록된 SHA-256과 일치했습니다.
- 동봉 Governor는 공식 v0.4.11 커밋 `60ab6e5b354f01f287c73d920990dcd618a674cc`에서 다시 빌드했으며, 전체 수정 차이는 `vendor/patches/`에 보존했고 격리 빌드에서 Rust 테스트 37개가 모두 통과했습니다.
- 동봉 UMR의 GNU build ID가 [Fedora 43 `1.0.10-6.fc43` 패키지 기록](https://packages.fedoraproject.org/pkgs/umr/umr/fedora-43.html)과 일치했습니다.
- 2026-08-02 Microsoft Defender 검사에서 프로젝트 파일에 대한 위협은 발견되지 않았습니다.
- 소스에서 비밀키, API 토큰 또는 외부 네트워크 리스너는 발견되지 않았습니다.
- 대상 BC-250 Bazzite 시스템에서 GTK4 환경 검사와 14개 시스템 파일 설치 재현 검사를 포함한 단위 테스트 171개가 모두 통과했습니다.

위 결과만으로 동봉 바이너리가 절대 안전하다고 증명되는 것은 아닙니다. 실행본 자체는 전체 소스 감사를 거치지 않았으며, 바이러스 검사와 해시 일치는 각각 알려진 위협 탐지와 파일 동일성만 확인합니다. 공개 릴리스에는 서명된 태그·체크섬 또는 배포판 서명을 사용하는 것이 좋습니다.

## 저작권과 재배포

- `cyan-skillfish-governor-smu`와 UMR의 MIT 원문 및 저작권 고지는 `vendor/licenses/`와 [`THIRD_PARTY_NOTICES.md`](THIRD_PARTY_NOTICES.md)에 포함되어 있습니다.
- **현재 프로젝트 루트에는 프로젝트 자체 `LICENSE` 파일이 없습니다.** 따라서 프로젝트 고유 코드에는 기본 저작권이 적용되고, 제3자에게 사용·수정·재배포할 명시적 권한이 부여되지 않은 상태입니다. Git 저장소를 공개 배포하기 전에 저작권자가 프로젝트 라이선스를 선택해 추가해야 합니다. 자세한 기본 동작은 [GitHub의 저장소 라이선스 안내](https://docs.github.com/en/repositories/managing-your-repositorys-settings-and-features/customizing-your-repository/licensing-a-repository)를 참고하십시오.
- 사전 빌드된 Governor 실행본은 정적으로 포함된 Rust 의존성의 전체 라이선스 목록이나 SBOM이 별도로 정리되어 있지 않습니다. 바이너리를 제3자에게 배포하려면 전이 의존성 라이선스를 확인하고 필요한 고지를 추가해야 합니다.
- 이 저장소는 AMD, Bazzite, Fedora 또는 동봉 도구 개발자의 공식 제품이 아닙니다. 각 이름과 상표의 권리는 해당 소유자에게 있습니다.

공개 배포 전 최소 점검 목록:

- [ ] Polkit `AUTH_ADMIN_KEEP` 제거 또는 전용 동작으로 교체
- [ ] 거버너 D-Bus 쓰기 권한 제한
- [ ] 수동 `install-umr` 다운로드 제거 또는 버전·해시 고정
- [ ] 프로젝트 자체 `LICENSE` 추가
- [ ] 동봉 바이너리의 전이 라이선스와 SBOM 정리
- [ ] Bazzite 대상 장치에서 설치·제거·재부팅 테스트 재실행

## 프로젝트 구조

```text
app.py                    GTK4 애플리케이션 진입점
bc250/                    상태, 환경 검사, 제어와 GUI 모듈
bc250_install.py          검증된 번들 설치·제거 도우미
bc250_privileged.py       제한된 영구 설정 도우미
vendor/                   버전 고정 실행본, 소스 패치, 서비스 템플릿과 라이선스
VENDOR-MANIFEST.json      설치 파일 해시와 출처
screenshot.png            Bazzite GNOME 실제 실행 화면
README.ko.md              한국어 문서
run.sh                    위치 독립 실행기
install-app.sh            GNOME 앱 목록 등록
uninstall-app.sh          GNOME 앱 목록 등록 제거
tests/                    unittest 및 실행 검증
```

## 복구와 제거

영구 설정을 바꾸기 전에 다음 백업을 만듭니다.

- `/etc/cyan-skillfish-governor-smu/config.toml.bc250-backup`
- `/etc/cyan-skillfish-governor-smu/config.toml.bc250-pre-range-update`
- `/etc/bc250-cu-live-manager.conf.bc250-backup`

앱 목록 등록만 제거하려면 다음 파일을 실행합니다.

```bash
./uninstall-app.sh
```

번들로 설치한 시스템 구성요소를 제거하려면 프로젝트 폴더에서 다음 설치기 작업을 실행할 수 있습니다.

```bash
pkexec python3 ./bc250_install.py remove --project-root "$PWD"
```

제거기는 매니페스트에 명시한 파일만 대상으로 하며 설정 백업은 보존합니다. 프로젝트 폴더 자체는 삭제하지 않습니다.

## 문제 해결

### GUI가 열리지 않음

`./run.sh --check`로 Python, GTK4, 번들 해시와 지원 플랫폼 판정을 확인합니다. Bazzite GNOME 이미지에는 필요한 Python GTK4 바인딩이 기본 제공됩니다.

### 번들 SHA-256 오류

파일이 손상되거나 변경된 상태입니다. 시스템 설치를 진행하지 말고 Git에서 깨끗한 사본을 다시 받으십시오. 해시 검사를 끄는 옵션은 제공하지 않습니다.

### UMR 호환 실패

현재 Bazzite 기반 버전의 LLVM 등 공유 라이브러리가 번들 실행본과 맞지 않는 상태입니다. 프로그램이 안내하는 배포판 패키지 설치 경로를 사용하고 rpm-ostree가 요구하면 재부팅하십시오.

### CU 수가 `확인 불가`

이번 부팅의 `bc250-cu-live-manager.service` 성공 로그와 저장 마스크를 모두 확인할 수 있을 때만 확정값을 표시합니다. 추정값을 40 CU라고 표시하지 않습니다.

### 온도·전력·클럭이 `확인 불가`

AMDGPU hwmon 경로가 없거나 드라이버가 해당 센서를 공개하지 않은 경우입니다. 화면이 정상 출력되더라도 누락된 센서값은 임의로 만들지 않습니다.

### 거버너 적용 실패

다른 거버너가 동시에 실행 중인지 확인하십시오. 이 프로그램은 `cyan-skillfish-governor-smu.service`만 제어 대상으로 사용합니다.

## 테스트

호스트 시스템 설정을 바꾸지 않는 전체 테스트는 다음과 같이 실행합니다.

```bash
./tests/run-tests.sh
./run.sh --check
```

권한 도우미 테스트는 임시 루트 디렉터리를 사용하므로 실제 `/etc`, systemd 서비스나 GPU 레지스터를 수정하지 않습니다.

## 주의사항

BC-250의 CU 활성화, 클럭과 전압 변경은 시스템 멈춤, 화면 손실, 데이터 손상, 발열 증가를 일으킬 수 있습니다. 안정적인 전원과 능동 냉각을 사용하고 중요한 작업을 저장한 뒤 설정을 변경하십시오. 프로그램은 사용자가 고른 값을 전달하고 영구 설정 변경 시 백업을 만들지만, 모든 개별 보드에서 해당 값이 지원되거나 안정적이라고 보증하지 않습니다.
